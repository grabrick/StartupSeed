...
const LoginPopup = () => {
  ...
  const [trowError, setTrowError] = useState('')
  ...
  const onSubmit = async (value) => {
    ...
    .then((response) => {
      ...
      console.log(login);
      auth.login(login.token, login.userID, login.isAdmin);
      setIsError(false)
      setTrowError('')
    }).catch(e => {
      setIsError(true)
      setTrowError(e.response.data.message)
    })
  };
            ...
            <Form
              ...
              render={({ handleSubmit }) => (
                <form onSubmit={handleSubmit} className={m.form}>
                  ...
                  {isError && <span style={{color: 'red'}}>{trowError}</span>}
